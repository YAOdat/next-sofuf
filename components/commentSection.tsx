...
const CommentSection: React.FC<CommentSectionProps> = ({ postID }) => {
  ...
  const [error, setError] = useState(null); // Store any errors during deletion
    ...
    useEffect(() => {
        ...
        // const determinePostID = () => {
        //     // Use a more robust router library for reliable route extraction
        //     const pathname = window.location.pathname; // Get current pathname
      
        //     if (pathname.match(/^\/resources\/whatsapp\/english-whatsapp-group$/)) {
        //       setPostID('whatsapp'); 
        //     }
        // };
    // delete comment
        ...
        <div>
            <>
                ...
                {commentsList.map((comment: DocumentData, index: React.Key | null | undefined) => (
                    <Card key={index} className="p-2 mt-4">
                        <CardHeader>
                            <Badge
                                content="Guest"
                                size="md"
